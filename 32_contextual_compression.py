...
bm25_results = bm25_retriever.invoke(rewritten_query)
...
vector_results = vector_retriever.invoke(rewritten_query)
...
# 24. CONTEXTUAL COMPRESSION PROMPT

compression_prompt = ChatPromptTemplate.from_template(
    """
You are a contextual compression system for a RAG pipeline.

Your task is to extract ONLY the information from the
document that is relevant to answering the user's question.

Rules:
- Keep only information relevant to the question.
- Preserve important facts, numbers, names and relationships.
- Remove unrelated information.
- Do not add new information.
- Do not answer the user's question.
- Return only the compressed relevant information.
- If the document contains nothing relevant, return:
  NOT_RELEVANT
...
# 25. CREATE COMPRESSION CHAIN

compression_chain = (compression_prompt | llm | StrOutputParser())
print("Contextual Compression initialized")

# 26. COMPRESS RERANKED DOCUMENTS
...
for doc, score in top_results:
    ...
    if compressed_text.strip() != "NOT_RELEVANT":
        ...
        compressed_results.append(compressed_text)

# 27. DISPLAY COMPRESSED RESULTS

print("\n========================================")
print("COMPRESSED RESULTS")
print("========================================")

for i, compressed_text in enumerate(compressed_results,1):

    print(f"\nCompressed Result {i}:")
    print(compressed_text)

# 28. BUILD COMPRESSED CONTEXT

context = "\n\n".join(compressed_results)

# 29. FINAL ANSWER PROMPT
...
# 30. FINAL ANSWER CHAIN

answer_chain = (answer_prompt | llm | StrOutputParser())

# 31. GENERATE FINAL ANSWER
